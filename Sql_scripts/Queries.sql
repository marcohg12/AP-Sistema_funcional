...
Borra el usuario
da -1 si no se puede borrar
-5 si el usuario no existe
0 si se logra borrar.
Busca por usuario.
*/
DELIMITER //
DROP PROCEDURE IF EXISTS delete_user;
CREATE PROCEDURE delete_user(in username varchar(50), OUT execution_code INT)
BEGIN
DECLARE check_id INT; 
...
BEGIN
    ...
    ROLLBACK;
END;
SELECT COUNT(*) INTO check_id FROM user_table WHERE user_table.username = username;
IF (check_id = 0) THEN
SET execution_code =-5;
SIGNAL CUSTOM_EXCEPTION;
END IF;
select id into personid from person where person.user_ref = username;
delete from person_x_nationality where person_ref = personid;
delete from telephone where person_ref = personid;
delete from email where person_ref = personid;
delete from admin where admin.username = username;
delete from person where person.user_ref = username;
delete from reservation where reservation.user_ref = username;
delete from hotel_x_user where user_ref = username;
DELETE FROM user_table WHERE user_table.username = username;
SET execution_code = 0;
COMMIT;
END
// 